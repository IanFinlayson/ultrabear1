...
Sim::Image::Image( const char* fileName )
{
	...
	{
    char reason[256];
    strcpy(reason, "Could not load: ");
    strcat(reason, fileName);
		throw Sim::Exception( "Image Constructor", reason);
	}

	image = SDL_DisplayFormat( temp );
  if(image == NULL) throw Sim::Exception("Image Constructor", "Couldn't optimize image");
