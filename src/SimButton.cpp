...
Sim::Button::Button( int x, int y, int w, int h, int borderSize, int border_r, int border_g, int border_b, int x_offset, int y_offset, bool trans )
{
	...
	is_pushed = false;
	transparent = trans;
  back.image = NULL; back.color = NULL; fore.image = NULL; fore.text = NULL;
}
...
void Sim::Button::ChangeBackColor( int r, int g, int b )
{
	if( !back_is_image )
	{
		back.color->r = r;
...
{
	// First we draw the border...if were' not transparent...
	if( !transparent )
